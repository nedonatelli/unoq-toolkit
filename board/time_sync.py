#!/usr/bin/env python3
"""Sync system time to STM32 Binary Clock sketch via Bridge RPC."""
...
def main():
    bridge = SimpleBridge()
    bridge.connect()
    # Wait for sketch to boot and register RPC method
    time.sleep(2)

    now = datetime.now()
    h, m, s = now.hour, now.minute, now.second
    result = bridge.call("set_time", h, m, s)
    print(f"Time synced: {h:02d}:{m:02d}:{s:02d} (result={result})")
