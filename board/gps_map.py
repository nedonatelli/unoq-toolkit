#!/usr/bin/env python3
"""GPS Map Display — fetches Esri satellite imagery and pushes to TFT via Bridge RPC.

Runs on the UNO Q's Linux side. Polls the MCU for GPS coordinates via
the 'get_gps' RPC method, fetches an Esri World Imagery tile centered
on the position, converts to RGB565, and pushes row-by-row via 'set_row'.
...
TFT_W, TFT_H = 240, 135
IMG_W, IMG_H = 240, 135  # Native TFT resolution, no scaling
...
def latlon_to_tile(lat, lon, zoom):
    """Convert lat/lon to tile x, y and pixel offset within tile."""
    n = 2 ** zoom
...
def main():
    print("GPS Map Display")
    print("=" * 40)
    print(f"TFT: {TFT_W}x{TFT_H}, Image: {IMG_W}x{IMG_H} (native), Zoom: {ZOOM}")
    print()
    ...
    while True:
        try:
            # Get GPS coordinates from MCU
            result = bridge.call("get_gps", "", timeout=10)
            print(f"  get_gps returned: {repr(result)}", flush=True)
            if result is None or result == "nofix":
                if last_lat is None:
                    # Use last known position as fallback for testing
                    print("No fix — using fallback coordinates", flush=True)
                    result = "38.8573000,-77.3822000"
                else:
                    print("No GPS fix yet, waiting...", flush=True)
                    time.sleep(5)
                    continue

            parts = result.split(",")
            if len(parts) != 2:
                print(f"Unexpected GPS response: {result}", flush=True)
                time.sleep(5)
                continue
            ...
            # Check if we've moved enough to refresh
            if last_lat is not None:
                dlat = abs(lat - last_lat)
                dlon = abs(lon - last_lon)
                if dlat < MOVE_THRESHOLD and dlon < MOVE_THRESHOLD:
                    print(f"  Position stable, next check in {REFRESH_INTERVAL}s")
